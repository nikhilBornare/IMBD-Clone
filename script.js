...
function renderList(actionForButton) {
  ...
  for (let i = 0; i < currentMovieStack.length; i++) {
    // creating div element for movie card and setting class
...
// when we clicked on home button come to home page
...
function favourite(element) {
  ...
  for (let i = 0; i < currentMovieStack.length; i++) {
    if (currentMovieStack[i].id == id) {
      let favouriteMoviesNikhil = JSON.parse(
        localStorage.getItem("favouriteMoviesNikhil")
      );

      if (favouriteMoviesNikhil == null) {
        favouriteMoviesNikhil = [];
      }

      favouriteMoviesNikhil.unshift(currentMovieStack[i]);
      localStorage.setItem(
        "favouriteMoviesNikhil",
        JSON.stringify(favouriteMoviesNikhil)
      );
...
goToFavouriteButton.addEventListener("click", () => {
  ...
  if (favouriteMoviesNikhil == null || favouriteMoviesNikhil.length < 1) {
    showAlert("you have not added any movie to favourite");
    return;
  }

  currentMovieStack = favouriteMoviesNikhil;
  renderList("remove");
});
...
function remove(element) {
  ...
  for (let i = 0; i < favouriteMoviesNikhil.length; i++) {
    if (favouriteMoviesNikhil[i].id == id) {
      continue;
    }
    newFavouriteMovies.push(favouriteMoviesNikhil[i]);
  }
